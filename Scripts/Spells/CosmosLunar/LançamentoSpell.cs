...
{
	public class LancamentoSpell : CosmosLunarSpell
	{
        ...
        public LancamentoSpell( Mobile caster, Item scroll ) : base( caster, scroll, m_Info )
		{
		}
		...
		public void Target( Mobile m )
		{
			...
			else if ( CheckHSequence( m ) && CheckFizzle() )
			{
				if ( GetSword() != null )
				{
					...
					int max = (int)(bw.MaxDamage + (GetCosmosLunarDamage( Caster ) / 7 ));
		...
		{
			private LancamentoSpell m_Owner;

			public InternalTarget(LancamentoSpell owner ) : base( Core.ML ? 10 : 12, false, TargetFlags.Harmful )
			{
				m_Owner = owner;
			}
