...
{
	public class BaseArmor_ItemOfLight : BaseArmor
    {
		private Item mLight;
		bool mIsLit = false;
		...
		[Constructable]
        public BaseArmor_ItemOfLight() : base( 0x1415 )
        {
		...
		public override void OnDoubleClick(Mobile from) 
		{
			...
			if( from.FindItemOnLayer(Layer.Face) != mLight ) {
				from.SendMessage("You cannot do that at this time.");
				return;
			}
			
			if( IsLit == false & from.FindItemOnLayer(Layer.Face) == null ) {
				IsLit = true;
				CreateLight(from);
			}
			else{ Nullify(); }
        ...
        public BaseArmor_ItemOfLight(Serial serial) : base(serial)
        { }
