    ...
    public class RevealSpell : MagerySpell
    {
        ...
        public void Target(IPoint3D p)
        {
            ...
            else if (CheckSequence())
            {
                ...
                if (map != null)
                {
                    IPooledEnumerable eable = map.GetMobilesInRange(new Point3D(p), 1 + (int)(Caster.Skills[SkillName.Arcanismo].Value / 20.0));

                    foreach (Mobile m in eable)
                    {
                        if ((m is Mobiles.ShadowKnight && (m.X != p.X || m.Y != p.Y)) || !SkillHandlers.Percepcao.CanDetect(Caster, m))
                            continue;
        ...
        private static bool CheckDifficulty(Mobile from, Mobile m)
        {
            ...
            int magery = from.Skills[SkillName.Arcanismo].Fixed;
