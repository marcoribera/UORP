...
{
	public class IlusaoSpell : CosmosLunarSpell
	{
        ...
        public IlusaoSpell( Mobile caster, Item scroll ) : base( caster, scroll, m_Info )
		{
		}
...
{
	public class IlusaoSpell : BaseCreature
	{
		private Mobile m_Caster;

		public IlusaoSpell( Mobile caster ) : base( AIType.AI_Melee, FightMode.Closest, 10, 1, 0.2, 0.4 )
		{
			m_Caster = caster;
			...
			{
				AddItem( IlusaoItem( caster.Items[i] ) );
			}
			...
			int min = 60;
			int max = (int)(Server.Spells.CosmosLunar.CosmosLunarSpell.GetCosmosLunarDamage( m_Caster ) );
				if ( max < min ){ max = min; }
			...
			TimeSpan duration = TimeSpan.FromSeconds( Server.Spells.CosmosLunar.CosmosLunarSpell.GetCosmosLunarDamage( m_Caster ) / 2 );
		...
		private Item IlusaoItem( Item item )
		{
		...
		public IlusaoSpell( Serial serial ) : base( serial )
		{
		}
