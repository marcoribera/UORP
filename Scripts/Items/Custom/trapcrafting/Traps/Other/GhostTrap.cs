...
namespace Server.Items
{
    public class GhostTrap : BaseTinkerTrap
    {
        ...
        private static string m_ArmedName = "Uma armadilha de fantasma armada";
        private static string m_UnarmedName = "Uma armadilha de fantasma desarmada";
        ...
        public override void TrapCheckTrigger(Mobile from)
        {
            if ( from.Body == 0x99 || from.Body == 0x1A )  // Body Types for Ghost Type Mobiles
                if ((Utility.Random(100) + 1) > from.Skills.ResistenciaMagica.Base)
                    TrapEffect( from );
                else
                    from.PlaySound(0x4A);
        }
