    ...
    class Research
    {
		...
		public static void SetInk( ResearchBag bag, Mobile from ) //////////////////////////////////////////////////////////////////////////////
		{
			...
			else
			{
				...
				if ( from.Skills[SkillName.Alquimia].Value >= Utility.RandomMinMax( 25, 150 ) ){ qty++; }
				if ( from.Skills[SkillName.Culinaria].Value >= Utility.RandomMinMax( 25, 150 ) ){ qty++; }
				if ( from.Skills[SkillName.Culinaria].Value >= Utility.RandomMinMax( 25, 150 ) ){ qty++; }
		...
		public static void CreateNormalSpell( ResearchBag bag, Mobile from, int spell ) /////////////////////////////////////////////////////////////
		{
			...
			else if ( from.Skills[SkillName.Erudicao].Value < skill )
			{
			...
			else if ( from.Skills[SkillName.Erudicao].Value < Utility.RandomMinMax( skill-25, skill+25 ) && canMake )
			{
				from.CheckSkill( SkillName.Erudicao, 0, 125 );
				canMake = false;
				if ( from.Skills[SkillName.Erudicao].Value < Utility.RandomMinMax( 0, 125 ) ){ from.Mana = from.Mana - mana; }
				if ( from.Skills[SkillName.Erudicao].Value < Utility.RandomMinMax( 0, 125 ) ){ bag.BagInk = bag.BagInk - 1; if ( bag.BagInk < 1 ){ bag.BagInk = 0; } }
				if ( from.Skills[SkillName.Erudicao].Value < Utility.RandomMinMax( 0, 125 ) ){ bag.BagScrolls = bag.BagScrolls - 1; if ( bag.BagScrolls < 1 ){ bag.BagScrolls = 0; } }
				if ( from.Skills[SkillName.Erudicao].Value < Utility.RandomMinMax( 0, 125 ) ){ bag.BagQuills = bag.BagQuills - 1; if ( bag.BagQuills < 1 ){ bag.BagQuills = 0; } }
				if ( from.Skills[SkillName.Erudicao].Value < Utility.RandomMinMax( 0, 125 ) ){ CheckReagents( from, spell, true, false, 1 ); }
		...
		public static void CreateResearchSpell( ResearchBag bag, Mobile from, int spell ) ///////////////////////////////////////////////////////////
		{
			...
			else if ( from.Skills[SkillName.Erudicao].Value < Utility.RandomMinMax( skill-25, skill+25 ) && canMake )
			{
				...
				if ( from.Skills[SkillName.Erudicao].Value < Utility.RandomMinMax( 0, 125 ) ){ bag.BagScrolls = bag.BagScrolls - 1; if ( bag.BagScrolls < 1 ){ bag.BagScrolls = 0; } }
				if ( from.Skills[SkillName.Erudicao].Value < Utility.RandomMinMax( 0, 125 ) ){ bag.BagQuills = bag.BagQuills - 1; if ( bag.BagQuills < 1 ){ bag.BagQuills = 0; } }
				if ( from.Skills[SkillName.Erudicao].Value < Utility.RandomMinMax( 0, 125 ) ){ CheckReagents( from, spell, true, true, 1 ); }
				from.PlaySound(0x249);
				msg = "You fail to scribe the " + name + " scroll and some of your materials may be lost.";
			}
		...
		public static void CreateManySpells( ResearchBag bag, Mobile from, int spell ) ///////////////////////////////////////////////////////////
		{
			...
			while ( !stopAll )
			{
				...
				else if ( from.Skills[SkillName.Erudicao].Value < Utility.RandomMinMax( skill-25, skill+25 ) && canMake )
				{
					from.CheckSkill( SkillName.Erudicao, 0, 125 );
					canMake = false;
					if ( from.Skills[SkillName.Erudicao].Value < Utility.RandomMinMax( 0, 125 ) && manaCheck ){ from.Mana = from.Mana - mana; manaCheck = false; }
					if ( from.Skills[SkillName.Erudicao].Value < Utility.RandomMinMax( 0, 125 ) ){ bag.BagScrolls = bag.BagScrolls - 1; if ( bag.BagScrolls < 1 ){ bag.BagScrolls = 0; } }
					if ( from.Skills[SkillName.Erudicao].Value < Utility.RandomMinMax( 0, 125 ) ){ bag.BagQuills = bag.BagQuills - 1; if ( bag.BagQuills < 1 ){ bag.BagQuills = 0; } }
					if ( from.Skills[SkillName.Erudicao].Value < Utility.RandomMinMax( 0, 125 ) ){ reagents++; }
					playSound = true;
