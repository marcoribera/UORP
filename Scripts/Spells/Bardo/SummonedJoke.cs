...
namespace Server.Mobiles
{
	...
	public class SummonedJoke : BaseCreature
	{
		...
		public SummonedJoke( Mobile owner, int skills, int time, int str, int dex, int iq, int poisons ): base( AIType.AI_Melee, FightMode.Closest, 10, 1, 0.3, 0.6 )
		{
			...
			SetSkill( SkillName.Envenenamento, (double)skills );
			SetSkill( SkillName.Anatomia, (double)skills );
			SetSkill( SkillName.ResistenciaMagica, (double)skills );
			SetSkill( SkillName.PreparoFisico, (double)skills );
			SetSkill( SkillName.Briga, (double)skills );
		...
		public static void MakeJoker( Mobile from, Point3D p, int poisons, string name, int body, int hue, int sound, int phys, int cold, int fire, int pois, int engy )
		{
			...
			int TotalTime = Server.Spells.Bardo.BardoSpell.Buff( from, "time" );
			int TotalBuff = Server.Spells.Bardo.BardoSpell.Buff( from, "strength" );
			int TotalPoison = Server.Spells.Bardo.BardoSpell.Buff( from, "poison" );
				if ( poisons < 1 ){ TotalPoison = 0; }
			int TotalSkill = Server.Spells.Bardo.BardoSpell.Buff( from, "skills" );
			int TotalDamage = Server.Spells.Bardo.BardoSpell.Buff( from, "damage" );
