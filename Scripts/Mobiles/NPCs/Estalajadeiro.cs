...
{
	public class Estalajadeiro : BaseVendor
	{
		private readonly List<SBInfo> m_SBInfos = new List<SBInfo>();

		[Constructable]
		public Estalajadeiro()
			: base("O Taverneiro")
		{
			SetSkill(SkillName.Carisma, 100.0, 100.0);
			SetSkill(SkillName.Percepcao, 90.0, 100.0);
			SetSkill(SkillName.Culinaria, 65.0, 88.0);
		}

		public Estalajadeiro(Serial serial)
			: base(serial)
		{ }
		...
		{
			m_SBInfos.Add(new SBEstalajadeiro());
		}
        ...
        public override void OnMovement(Mobile m, Point3D oldLocation)
        {
            if (PetTrainingHelper.Enabled && m.Alive && !m.Hidden && m is PlayerMobile)
            {
                ...
                if (InLOS(m) && InRange(m, 8) && !InRange(oldLocation, 8) && DateTime.UtcNow >= _NextTalk)
                {
                    ...
                    _NextTalk = DateTime.UtcNow + TimeSpan.FromSeconds(50);
                }
            }
        }*/
      ...
      */
		...
		{
			var taming = from.Skills[SkillName.Carisma].Value;
			var anlore = from.Skills[SkillName.Percepcao].Value;
			var vetern = from.Skills[SkillName.Culinaria].Value;
		...
		public void BeginClaimList(Mobile from)
		{
			...
			{
				SayTo(from, 502671); // Nenhum companheiro seu veio se dormir aqui hoje
            }
		}
		...
		public void EndClaimList(Mobile from, BaseCreature pet)
		{
			...
			{
				SayTo(from, 1049612, pet.Name); // ~1_NAME~ ficou na estalagem por que você já tem muitos seguidores
            }
		}
		...
		public void BeginStable(Mobile from)
		{
            ...
            /* 
			 *Eu cobro 30 de ouro por cada noite que seus companheiros passarem aqui.
			 *Vou retirá-lo de sua conta bancária. Você quer que eu prepare a cama para quem?
			 */
		...
		public void EndStable(Mobile from, BaseCreature pet)
		{
			...
			{
				SayTo(from, 1042565); // Nesse momento você já está ocupando todos os quartos disponíveis
            }
		...
		private class StableEntry : ContextMenuEntry
		{
			...
			private readonly Estalajadeiro m_Trainer;

			public StableEntry(Estalajadeiro trainer, Mobile from)
				: base(6126, 12)
			{
				m_Trainer = trainer;
				m_From = from;
		...
		private class ClaimListGump : Gump
		{
			...
			public ClaimListGump(Estalajadeiro trainer, Mobile from, List<BaseCreature> list)
				: base(50, 50)
			{
				m_Trainer = trainer;
				m_From = from;
		...
		private class ClaimAllEntry : ContextMenuEntry
		{
			...
			public ClaimAllEntry(Estalajadeiro trainer, Mobile from)
				: base(6127, 12)
			{
				m_Trainer = trainer;
				m_From = from;
		...
		private class StableTarget : Target
		{
			...
			public StableTarget(Estalajadeiro trainer)
