    ...
    public class WesternWargRider : BaseCreature
    {
        ...
        public override WeaponAbility GetWeaponAbility()
        {
            switch (Utility.Random(2))
            {
                ...
                case 1: return WeaponAbility.ConcussionBlow;
            }
        }
        ...
        public WesternWargRider() : base(AIType.AI_Melee, FightMode.Closest, 10, 1, 0.2, 0.4)
        {
            ...
            SetResistance(ResistanceType.Energy, 40, 45); SetSkill(SkillName.ResistenciaMagica, 84.5, 97.5);
            SetSkill(SkillName.Cortante, 97.5, 119.5); SetSkill(SkillName.Anatomia, 97.5, 119.5);
        ...
        {
            if (this.m_NextFastHeal < DateTime.Now)
            {
                if (this.Hits < this.HitsMax)
                {
                    this.Hits += (this.HitsMax / 6);
                    this.m_NextFastHeal = DateTime.Now + TimeSpan.FromSeconds(13);
                }
            }
        }
        public override bool OnBeforeDeath()
        {
            IMount mount = this.Mount; if (mount != null)
            {
                mount.Rider = null;
            }
            return base.OnBeforeDeath();
        }
        public WesternWargRider(Serial serial) :
base(serial)
        { }
