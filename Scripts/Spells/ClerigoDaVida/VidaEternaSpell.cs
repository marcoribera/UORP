...
{
    public class VidaEternaSpell : ClerigoDaVidaSpell
    {
        private static readonly SpellInfo m_Info = new SpellInfo(
            "Vida Eterna", "Vita Aeterna",
            221,
            9002,
            Reagent.Bloodmoss,
            Reagent.SpidersSilk,
        ...
        public VidaEternaSpell(Mobile caster, Item scroll, int body)
            : base(caster, scroll, m_Info)
        {
            this.m_NewBody = body;
        }

        public VidaEternaSpell(Mobile caster, Item scroll)
        ...
        public override bool CheckCast()
        {
            ...
            else if (!this.Caster.CanBeginAction(typeof(VidaEternaSpell)))
            {
                if (Core.ML)
                    EndPolymorph(this.Caster); ////endpolymorph
                else 
        ...
        public override void OnCast()
        {
            ...
            {
                if (this.Caster.BeginAction(typeof(VidaEternaSpell)))
                {
                    if (this.m_NewBody != 0)
                    {
        ...
        public static void EndPolymorph(Mobile m) // EndPolymorph
        {
            if (!m.CanBeginAction(typeof(VidaEternaSpell)))
            {
                m.BodyMod = 0;
                m.HueMod = -1;
                m.EndAction(typeof(VidaEternaSpell));
